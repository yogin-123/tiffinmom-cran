  ...
  tiffin_detail(params) {
    return new Promise((resolve, reject) => {
      connection.query(`SELECT CONCAT('${S3_URL + TIFFIN_IMAGE}',image) AS image,id,title,delivery_on,price,description FROM tbl_tiffins WHERE is_active = 'Active' AND id = ${params.tiffin_id} LIMIT 1`, (error, result) => {
        if (!error && result[0]) {
          connection.query(`SELECT tc.id,tc.name,tc.quantity,tc.position FROM tbl_tiffin_relation as ttr left join tbl_tiffin_detail AS td on td.id = ttr.tiffin_detail_id left JOIN tbl_tiffin_category AS tc ON tc.id = ttr.category_id WHERE tc.is_active = 'Active' AND td.is_active = 'Active' AND ttr.tiffin_id = ${params.tiffin_id} GROUP BY ttr.category_id order by tc.position ASC`, (categoryError, categoryResult) => {
