            ...
            if (email) {
              con.query(`SELECT CONCAT(t.title," : ",IFNULL(GROUP_CONCAT(CONCAT(td.name,' ( Qty. ',otd.quantity,' )')),'')) AS name,od.quantity,t.price FROM tbl_orders AS o JOIN tbl_order_detail AS od ON od.order_id = o.id JOIN tbl_tiffins AS t ON t.id = od.tiffin_id JOIN tbl_order_tiffin_detail AS otd ON otd.detail_id = od.id JOIN tbl_tiffin_detail AS td ON td.id = otd.category_id JOIN tbl_tiffin_category AS tc ON tc.id = td.category_id WHERE o.is_active = 'Active' AND o.id = ${insertResult.insertId} AND tc.quantity != 0 GROUP BY od.id ORDER BY od.id DESC;`, (tiffinOrderError, tiffinOrderResult) => {
                con.query(`SELECT p.name,od.quantity,od.price FROM tbl_orders AS o JOIN tbl_order_detail AS od ON od.order_id = o.id JOIN tbl_product AS p ON p.id = od.product_id WHERE o.is_active = 'Active' AND o.id = ${insertResult.insertId} GROUP BY od.id ORDER BY od.id DESC;`, (trayOrderError, trayOrderResult) => {
                  con.query(`SELECT td.name,otd.quantity,td.price FROM tbl_order_tiffin_detail AS otd JOIN tbl_tiffin_detail AS td ON td.id = otd.category_id JOIN tbl_tiffin_category AS tc ON tc.id = td.category_id AND tc.quantity = 0 WHERE otd.order_id = ${insertResult.insertId};`, (extraTiffinOrderError, extraTiffinOrderResult) => {
