...
module.exports = {
  ...
  kitchen_report(body) {
    return new Promise((resolve, reject) => {
      ...
      con.query(`select o.id,td.name,SUM(otd.quantity  * od.quantity) quantity from tbl_orders AS o JOIN tbl_order_detail AS od ON od.order_id = o.id JOIN tbl_order_tiffin_detail AS otd ON otd.detail_id = od.id join tbl_tiffin_relation as ttr on ttr.id = otd.category_id JOIN tbl_tiffin_detail AS td ON td.id = ttr.tiffin_detail_id JOIN tbl_tiffin_category AS tc ON tc.id = ttr.category_id AND tc.quantity != 0 where o.is_active = 'Active' ${where} ${tiffinWhere} group by otd.category_id order by SUM(otd.quantity) desc;`, (mainItemError, mainItemResult) => {
