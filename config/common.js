...
const common = {
    ...
    admin_validate_token(req, res, callback) {
        try {
            req.language = 'en';
            let path_data = req.path.split("/");
            let byPassMethod = new Array('signin');
    ...
    website_validate_token(req, res, callback) {
        try {
            req.language = 'en';
            let path_data = req.path.split("/");
            let byPassMethod = new Array('home_data', 'signin', 'signup', 'reset_password', 'send_otp', 'verify_otp', 'state_list', 'contact_us', 'gallery', 'product_list', 'category_list', 'faqs', 'how_it_work_page', 'tiffin_list', 'tiffin_detail', 'subscription_list');
    ...
    checkAdminDeviceInfo(id) {
        return new Promise((resolve, reject) => {
            let token = (new Date()).getTime() + common.stringGen(12) + common.numberGen(16);
            let device = {
                token: token,
